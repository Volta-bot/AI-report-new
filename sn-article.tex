...
\textbf{Scalability and Fairness:} The results confirm that even in inherently difficult regions, clients can achieve reasonable accuracy through federated knowledge access without sacrificing data sovereignty \cite{tariq2024trustworthy}. Future integration of personalized FL schemes \cite{liang2020think} and decentralized averaging \cite{sun2021pain} could further improve fairness across heterogeneous clients.
...
\begin{itemize}
    ...
    \item \textit{Bandwidth Optimization:} The framework achieved a transformative 262$\times$ reduction in communication overhead. Instead of streaming massive high-frequency raw datasets, the system only transmits compressed model parameters (weights) every round \cite{mcmahan2017fedavg}. Communication efficiency strategies \cite{konecny2016federated} such as gradient compression and quantization can reduce this overhead even further.
