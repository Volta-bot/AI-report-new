...
In each round $t$, the server selects a subset of clients $S_{t}$ and sends the current global model $w_{t}$ to these clients. Each client $k$ performs $E$ epochs of local training (SGD) to obtain $w_{t+1}^{k}$. Finally, the server aggregates the updates using the formula expressed as eq.8 \cite{mcmahan2017fedavg}:
