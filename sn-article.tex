...
This research presented a comprehensive, privacy-preserving Edge-based Traffic Prediction framework utilizing Federated Learning \cite{mcmahan2017fedavg}. By deploying a specialized, lightweight Multi-Horizon LSTM architecture \cite{hochreiter1997lstm} at the network edge, the system successfully forecasted short-term (15-minute), medium-term (30-minute), and long-term (60-minute) traffic conditions simultaneously. A key methodological innovation was the automated topological placement of edge clients using Louvain clustering \cite{blondel2008louvain} and centrality metrics, which naturally addressed spatial disparities in the road network. Through the Federated Averaging (FedAvg) algorithm \cite{mcmahan2017fedavg}, edge devices trained collaboratively by exchanging only model weights, ensuring that raw, sensitive mobility data remained strictly localized.

Experimental evaluations on the PeMS-BAY and METR-LA benchmark datasets \cite{li2018dcrnn} demonstrated the framework's high efficacy. The proposed federated model achieved highly competitive predictive performance, retaining 92\% to 96\% of the centralized baseline accuracy \cite{liu2020fedgru}. Most notably, this approach drastically optimized network efficiency, reducing communication bandwidth consumption by a massive factor of 80 to 262 times through the exchange of minimal 20\,KB weight files \cite{mcmahan2017fedavg}.

Despite these promising results, certain limitations remain. The current implementation relies on the standard FedAvg algorithm \cite{mcmahan2017fedavg}, which can experience slower convergence when faced with extreme non-IID (Independent and Identically Distributed) data distributions \cite{Zhao2018FederatedNonIID}. Furthermore, the continuous transmission of model weights still poses a communication overhead for highly resource-constrained edge devices. To address these operational challenges, future work will focus on integrating advanced optimization algorithms, such as FedProx or SCAFFOLD, to better handle statistical heterogeneity \cite{li2024fedasta}. Additionally, investigating Spatio-Temporal Graph Neural Networks (ST-GNNs) \cite{feng2024federated} and fusing multimodal data streams---including roadside camera feeds and real-time weather conditions---will further enhance the system's robustness, accuracy, and reliability for practical ITS deployments.
