...
In each round $t$, the server selects a subset of clients $S_{t}$ and sends the current global model $w_{t}$ to these clients. Each client $k$ performs $E$ epochs of local training (SGD) to obtain $w_{t+1}^{k}$. Finally, the server aggregates the updates using the formula expressed as eq.\eqref{eq:8} \cite{mcmahan2017fedavg}:
...
% ============================================================
%  CHAPTER 3 — PROPOSED MODEL  (corrected, complete version)
% ============================================================
...
This chapter details the methodology for the Edge-based Traffic Prediction system
via Federated Learning (FL). The system is specifically designed to address the
critical challenges of privacy, bandwidth, and latency in Intelligent Transportation
Systems (ITS)~\cite{tariq2024trustworthy,bharati2022federated}. By dividing
different traffic regions into independent and isolated clients, the architecture
ensures that raw traffic data never leaves the local environment, thereby
guaranteeing robust data privacy~\cite{gdpr2016}.

To overcome the challenges of insufficient or poor-quality data at individual
nodes~\cite{qin2025fedcontrastive}, the system leverages FL to collaboratively
train a global model. Furthermore, it addresses the issue of model parameter bloat
by utilizing a lightweight architecture to reduce communication
burdens~\cite{konecny2016federated}, while incorporating mechanisms to handle
data imbalance among clients~\cite{Zhao2018FederatedNonIID}. This ensures that
clients with large amounts of data do not disproportionately overshadow the
individuality of other regions.

Beyond traffic speed prediction, the framework is further extended to support
\textbf{traffic congestion detection and management}---a critical operational
requirement for modern ITS. By deriving congestion-level indicators directly from
the multi-horizon speed forecasts produced at each edge node, the system enables
proactive, privacy-preserving congestion management without ever centralizing
raw sensor streams.
...
The framework operates on a distributed architecture consisting of two primary
roles: \textbf{Local Clients} and a \textbf{Central Server}. Local Clients
function as independent, autonomous computing units that manage local data.
They provide the necessary data resources by training the model with parameters
dispatched by the server and uploading the updated weights. Because the
architecture is highly flexible, the dynamic joining or exiting of these clients
does not significantly impact overall performance. The Central Server acts as the
overarching coordinator. To ensure strict data privacy, it operates completely
independently of the clients' raw data. Its primary responsibilities include
initializing the global model, distributing parameters, aggregating the uploaded
client updates, and generating the new global model for the next iteration.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{Figure/collaborative_training_process.png}
    \caption{The system architecture and training process of federated learning.}
    \label{fig:fl_framework}
\end{figure}

The collaborative training process follows a strict sequence in each communication
round~\cite{mcmahan2017fedavg}:

\begin{itemize}
    \item \textbf{Initialization \& Distribution:} The server initializes the
    global model weights $W_0$ and distributes the current weights $W_r$ to
    selected clients.

    \item \textbf{Local Training:} Each client $k$ uses its private local
    dataset $D_k$ to train the model to minimize prediction errors.

    \item \textbf{Upload \& Aggregation:} Clients upload their updated weights
    $W_r^k$ to the server. The server aggregates the weight parameters of the
    model transmitted by various clients to obtain a new global model.

    \item \textbf{Iteration:} This process repeats for a fixed number of rounds
    $R$ or until convergence is achieved.
\end{itemize}

For model aggregation, the server employs the weighted Federated Averaging
(FedAvg) algorithm~\cite{mcmahan2017fedavg} to ensure fair influence based on
local data size. In each round $r$, the server computes the new global model
...
\noindent where $S_r$ is the subset of selected clients, $n_k$ is the number of
samples on client $k$, and $n$ is the total number of samples.

% ------------------------------------------------------------
\subsection{System Architecture}
% ------------------------------------------------------------

The complete system is built upon five tightly integrated subsystems designed to
support both simulated and real-world data environments.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Traffic Data Environment}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

The traffic data source operates in two distinct modes:

\begin{itemize}
    \item \textbf{Simulation Mode:} Utilizes the SUMO (Simulation of Urban
    MObility) platform~\cite{lopez2018sumo} to generate realistic, multi-modal
    traffic flows on road networks imported from OpenStreetMap. SUMO exposes a
    programmatic \textbf{TraCI} (Traffic Control Interface) over TCP, enabling
    real-time extraction of high-frequency metrics---vehicle speed, count,
    density, occupancy, and flow rate---per road segment at each simulation
    timestep.

    \item \textbf{Real-World Mode:} Integrates benchmark datasets (PeMS-BAY and
    METR-LA)~\cite{li2018dcrnn} with 5-minute temporal resolutions for rigorous
    validation. This mode preserves the spatial heterogeneity of real sensor
    deployments and subjects the FL framework to authentic non-IID data
    distributions.
\end{itemize}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Data Streaming Layer}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

Real-time data flows from the traffic source to edge clients via a
\textbf{WebSocket}-based publish-subscribe architecture. The WebSocket publisher
subscribes to the SUMO TraCI interface and broadcasts aggregated traffic metrics
in structured JSON messages at approximately 1\,Hz. Each message carries the
edge identifier, timestamp, average speed, vehicle count, density, flow rate,
occupancy, and a per-type vehicle count breakdown.

Each edge client maintains a fixed-size \textbf{circular buffer} storing 300
seconds of aggregated data (using 10-second aggregation intervals, yielding 30
elements). This design guarantees $O(1)$ insertion time, prevents memory
reallocation during training, and ensures a continuously refreshed local training
set without requiring persistent storage.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Edge Clients and Federated Learning Server}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

Each \textbf{edge client} is an independent compute node responsible for data
reception, buffering, local model training, and federated participation. Local
training is performed using \textbf{PyTorch} with the \textbf{Adam optimizer}
(learning rate $10^{-3}$, batch size 32). Clients expose the
\texttt{flwr.client.NumPyClient} interface from the \textbf{Flower framework},
which defines four methods: \texttt{get\_parameters}, \texttt{set\_parameters},
\texttt{fit}, and \texttt{evaluate}.

The \textbf{FL server} coordinates training across all clients using the Flower
framework. Its configuration requires a minimum of 5 clients per round to proceed
with aggregation, applies a 300-second per-client timeout, and evaluates the
aggregated global model against each client's held-out test partition (20\% of
buffered data) after every round to monitor convergence.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Problem Definition}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

The traffic flow prediction task is formulated as learning a mapping function $f$
that predicts future traffic states from historical observations. Let
$X_t = \{x_{t-T_{in}+1}, \ldots, x_t\} \in \mathbb{R}^{T_{in} \times F}$ be
the input sequence of length $T_{in}$ at time $t$, where $F$ is the number of
features (e.g., speed, vehicle count, density). The goal is to predict the
multi-horizon output sequence:

\begin{equation}
    Y_t = \{y_{t+h_1},\, y_{t+h_2},\, \ldots,\, y_{t+h_m}\}
    \label{eq:problem_def}
\end{equation}

\noindent where $h_i$ represents specific look-ahead horizons corresponding to
short-term (3 steps / 15 min), medium-term (6 steps / 30 min), and long-term
(12 steps / 60 min) predictions.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Multi-Horizon LSTM Architecture}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

To capture temporal dependencies in traffic data, the system employs a
Long Short-Term Memory (LSTM) network~\cite{hochreiter1997lstm} optimized for
edge deployment:
...
The \textbf{LSTM encoder} consists of two stacked LSTM layers with 64 units
each. These layers process the temporal sequence and extract a latent
representation vector that summarizes the historical traffic pattern.

Instead of producing a single scalar output, the model features three distinct
\textbf{fully connected output heads}, each dedicated to one prediction horizon:

\begin{itemize}
    \item \textbf{Head 1 (Short-term):} Predicts traffic state at $t+3$
    (15 minutes ahead).
    \item \textbf{Head 2 (Medium-term):} Predicts traffic state at $t+6$
    (30 minutes ahead).
    \item \textbf{Head 3 (Long-term):} Predicts traffic state at $t+12$
    (60 minutes ahead).
\end{itemize}

Each head is a two-layer fully connected network ($64 \to 32 \to 1$). The
multi-head architecture enables \textbf{joint optimization} across all horizons
simultaneously, allowing shared temporal representations to improve short-term
accuracy while maintaining long-range predictive ability.

The model contains only \textbf{5,000 parameters} with a weight file size of
\textbf{20\,KB}, making it fully compatible with resource-constrained
\textbf{IoT edge devices}~\cite{shi2016edge,mach2017mobile}. Local training
requires approximately 12--15 seconds per round on a single CPU, with no GPU
required. This compactness is critical for deployment on roadside units (RSUs)
and embedded systems in real-world ITS environments.

% ------------------------------------------------------------
\subsection{Network Topology Analysis for Edge Placement}
% ------------------------------------------------------------

A crucial innovation in this methodology is the \textbf{automated, topology-based
placement} of edge clients within the road network, avoiding arbitrary manual
configurations.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Louvain Clustering for Community Detection}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

The system constructs an undirected graph $G = (V, E)$ of the road network, where
vertices $V$ represent road segments or intersections and edges $E$ encode
adjacency weighted inversely by inter-segment distance. \textbf{Louvain
clustering}~\cite{blondel2008louvain} is then applied to partition $G$ into
natural traffic communities by iteratively optimizing the modularity function:
...
\noindent where $A_{ij}$ is the adjacency weight, $k_i$ is the degree of vertex
$i$, $m$ is the total number of edges, $c_i$ is the community assignment of
vertex $i$, and $\delta(\cdot,\cdot)$ is the Kronecker delta. The algorithm
typically yields 4--10 natural traffic communities for urban networks comprising
40--100 road segments.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Centrality-Based Representative Selection}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

Within each detected community, the optimal edge node location is determined by
computing a \textbf{combined centrality score}. Two standard graph centrality
measures are employed:

\begin{itemize}
    \item \textbf{Betweenness Centrality} $C_B(v)$ measures how frequently
    segment $v$ lies on the shortest paths between all other pairs of segments:
    \begin{equation}
        C_B(v) = \sum_{s \neq v \neq t} \frac{\sigma_{st}(v)}{\sigma_{st}}
        \label{eq:betweenness}
    \end{equation}
    where $\sigma_{st}(v)$ is the number of shortest paths from $s$ to $t$ that
    pass through $v$, and $\sigma_{st}$ is the total number of shortest paths
    from $s$ to $t$.

    \item \textbf{Closeness Centrality} $C_C(v)$ measures the average proximity
    of segment $v$ to all other segments:
    \begin{equation}
        C_C(v) = \frac{n-1}{\sum_{t \neq v} d(v, t)}
        \label{eq:closeness}
    \end{equation}
    where $d(v, t)$ is the shortest-path distance from $v$ to $t$ and $n$ is the
    total number of vertices.
\end{itemize}

The final selection score is:
\begin{equation}
    \mathrm{Score}(v) = 0.5 \cdot C_B(v) + 0.5 \cdot C_C(v)
                      + 0.1 \cdot \mathrm{HistoricalVolume}(v)
    \label{eq:placement_score}
\end{equation}

The top-ranked node in each community is designated as the edge client location,
replicating the coverage area of a physical \textbf{Roadside Unit (RSU)}.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Benefits of Topology-Based Placement}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

This spatial partitioning strategy provides four key benefits:

\begin{enumerate}
    \item \textbf{Automatic configuration:} No manual specification is required;
    the algorithm adapts to any road network topology.
    \item \textbf{Balanced regional fairness:} Each client covers a
    topologically coherent traffic community, ensuring equitable knowledge
    representation in the federated aggregation.
    \item \textbf{Non-IID test realism:} Because communities naturally
    correspond to structurally different road types (e.g., highway vs.\
    arterial), the partitioning introduces the statistical heterogeneity
    required to rigorously test the FL algorithm against non-IID
    distributions~\cite{Zhao2018FederatedNonIID}.
    \item \textbf{Scalability:} The Louvain algorithm runs in $O(n \log n)$
    time, remaining practical for networks with hundreds of road segments.
\end{enumerate}

% ------------------------------------------------------------
\subsection{Traffic Congestion Detection and Management}
% ------------------------------------------------------------

While the core framework predicts multi-horizon traffic speed, a critical
extension---motivated by real-world ITS operational requirements---is the
derivation of \textbf{congestion-level indicators} from the predicted speed
sequences. This subsection describes how the proposed model is adapted to detect,
classify, and support the management of traffic congestion in a
privacy-preserving, edge-native manner.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Congestion Level Classification}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

At each edge client, the predicted traffic speeds $\hat{y}_{t+h}$ produced by
the Multi-Horizon LSTM are mapped to a \textbf{discrete congestion level}
$\mathcal{C} \in \{\text{Free-flow},\, \text{Moderate},\, \text{Congested},\,
\text{Severe}\}$ using a speed-threshold classifier calibrated to local road
characteristics. Formally, for a road segment with free-flow speed
$v_{\mathrm{ff}}$, the congestion level at horizon $h$ is defined as:

\begin{equation}
    \mathcal{C}_{t+h} =
    \begin{cases}
        \text{Free-flow}  & \text{if } \hat{y}_{t+h} \geq 0.75\, v_{\mathrm{ff}} \\
        \text{Moderate}   & \text{if } 0.50\, v_{\mathrm{ff}} \leq \hat{y}_{t+h} < 0.75\, v_{\mathrm{ff}} \\
        \text{Congested}  & \text{if } 0.25\, v_{\mathrm{ff}} \leq \hat{y}_{t+h} < 0.50\, v_{\mathrm{ff}} \\
        \text{Severe}     & \text{if } \hat{y}_{t+h} < 0.25\, v_{\mathrm{ff}}
    \end{cases}
    \label{eq:congestion_class}
\end{equation}

Because $v_{\mathrm{ff}}$ is learned from the local circular buffer of each edge
client, the thresholds are automatically calibrated to regional road conditions
(e.g., urban arterials versus freeways) without requiring global data sharing.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Federated Congestion Map Aggregation}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

Individual edge clients produce congestion-level forecasts for their assigned
road communities. The FL server aggregates these local predictions into a
\textbf{global congestion map} by collecting the predicted congestion labels
(not raw speed observations) from each client after every federated round. Since
only discrete labels or their probability distributions are transmitted---not raw
sensor readings---this aggregation step preserves the privacy guarantees already
established by the FedAvg framework.

The global congestion map is updated at each FL round and can be consumed by
downstream traffic management applications, such as adaptive signal control,
dynamic route recommendation, and incident alerting, all within latencies below
100\,ms at the edge.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Proactive Congestion Management Actions}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

By forecasting congestion levels across three horizons simultaneously, the system
supports a \textbf{tiered management response}:

\begin{itemize}
    \item \textbf{Short-term (15 min):} Immediate signal phase adjustments at
    intersections are triggered when a \textit{Congested} or \textit{Severe}
    label is predicted. Because predictions are computed locally on the edge
    client co-located with the traffic controller, the control latency is
    negligible compared to centralized query-response cycles.

    \item \textbf{Medium-term (30 min):} Anticipated congestion is broadcast to
    navigation applications as zone-level advisories, enabling drivers to
    self-select alternative routes before the bottleneck materializes.

    \item \textbf{Long-term (60 min):} Persistent congestion forecasts inform
    strategic decisions such as dynamic toll pricing, variable speed limit
    activation, and demand-side management strategies for public transit
    corridors.
\end{itemize}

This tiered response is illustrated in Figure~\ref{fig:congestion_mgmt}, which
depicts the information flow from edge-level LSTM predictions through congestion
classification to ITS actuation.

% Placeholder figure note:
% \begin{figure}[h!]
%     \centering
%     \includegraphics[width=0.85\textwidth]{Figure/congestion_management_flow.png}
%     \caption{Tiered congestion management flow: LSTM forecasts at each edge client
%     are mapped to congestion levels and aggregated by the FL server into a global
%     congestion map, which drives adaptive signal control, route advisories, and
%     demand management.}
%     \label{fig:congestion_mgmt}
% \end{figure}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Privacy Preservation in Congestion Management}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

The congestion management extension inherits all privacy-by-design properties of
the underlying FL framework. Raw traffic sensor streams---which encode individual
mobility patterns---remain strictly local at each edge client. Only
\textbf{model weight updates} (20\,KB per client per round) and the resulting
\textbf{congestion-level labels} are transmitted to the server. This design
ensures compliance with GDPR~\cite{gdpr2016} and CCPA~\cite{tariq2024trustworthy}
data minimization principles, while still enabling network-wide situational
awareness. Where additional formal guarantees are required, \textbf{differential
privacy}~\cite{dwork2014differential} noise can be added to the label probability
vectors before upload, and \textbf{secure aggregation}~\cite{bonawitz2017secagg}
protocols can be layered over the label collection step.

% ------------------------------------------------------------
\subsection{Communication Protocol and Efficiency}
% ------------------------------------------------------------

To handle the realities of network communication and bandwidth limitations, the
system incorporates several robust efficiency optimizations~\cite{tariq2024trustworthy}.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Asynchronous Communication Protocol}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

While each federated round is synchronous (the server waits for client
responses), the system is resilient to client-side delays through three
mechanisms:

\begin{enumerate}
    \item \textbf{Per-client timeout:} Each client has a 300-second window to
    complete local training and upload its weights. Clients exceeding this
    threshold are skipped for that round.
    \item \textbf{Minimum participant threshold:} Aggregation proceeds if at
    least 2 clients upload successfully, ensuring training continues even when
    a majority of clients are temporarily unreachable.
    \item \textbf{Exponential backoff:} If a client connection drops during
    upload, the server retries with exponential backoff intervals
    (1\,s, 5\,s, 25\,s) before timing out~\cite{qin2025fedcontrastive}.
\end{enumerate}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Bandwidth Efficiency Gains}
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

By transmitting only compressed model weight files of approximately 22\,KB per
client per round, the architecture achieves an \textbf{80--262$\times$ reduction}
in communication bandwidth compared to streaming raw sensor data to a centralized
server~\cite{mcmahan2017fedavg,liu2020fedgru}. This drastically lowers latency
and energy consumption for battery-powered or resource-constrained IoT devices.
Communication overhead is further minimized through:

\begin{itemize}
    \item \textbf{Local SGD strategies}~\cite{lin2017deep}: Performing multiple
    local gradient descent steps before each upload reduces the number of
    communication rounds required for convergence.
    \item \textbf{Matched averaging}~\cite{wang2019adaptive}: Neuron-level
    alignment of local models prior to aggregation reduces the gradient
    mismatch that arises from non-IID local data distributions.
    \item \textbf{Distillation-based aggregation}~\cite{park2020privacy}:
    Knowledge distillation at the server further compresses the information
    exchanged between rounds without sacrificing convergence quality.
\end{itemize}

The combined effect ensures that the entire FL pipeline---including the new
congestion label transmission---remains well within the bandwidth constraints of
standard roadside communication links (e.g., DSRC or C-V2X), making the proposed
framework viable for large-scale deployment in next-generation smart city ITS
infrastructure~\cite{ye2020edgefed,chen2019deep}.
...
Figures~\ref{fig:pred_pemsbay} and~\ref{fig:pred_metrla} illustrate how closely the model tracks ground-truth traffic speeds over a representative 200-sample window on each dataset. The predictions faithfully capture the general trend and respond quickly to abrupt speed drops (e.g., the congestion trough near sample indices 80--125), though the model slightly underestimates the sharpest troughs due to the inherent smoothing effect of the LSTM's temporal memory.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{Figure/pems-bay-predictions_vs_truth.png}
  \caption{Ground truth vs.\ predicted traffic speed on PeMS-BAY (MAE\,=\,1.374, RMSE\,=\,2.460). The model closely tracks the freeway speed profile, including the deep congestion event (samples 75--125), with minimal lag and accurate recovery.}
  \label{fig:pred_pemsbay}
\end{figure}
...
To validate the effectiveness of the proposed framework, the FL model was benchmarked against a centralized baseline (the ``Oracle'' scenario), where all historical traffic data is aggregated on a single server for training. The centralized reference results are drawn from a recent study on Spatio-Temporal Graph Neural Networks and Transformer architectures \cite{sttn2024}. Figure~\ref{fig:centralized_baseline} summarizes the performance of state-of-the-art centralized models on both benchmark datasets.

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{Figure/Performance-on-METR-LA-and-PEMS-BAY.png}
  \caption{Centralized baseline performance on METR-LA and PeMS-BAY across prediction horizons (3, 6, 12 steps). Results cited from \cite{sttn2024} cover models DCRNN, GWNet, AGCRN, MTGNN, STAEformer, ST\_4L, ST\_7L, and STTLM (best results highlighted in red). Our federated approach achieves competitive accuracy while providing strong privacy guarantees that none of these centralized models offer.}
  \label{fig:centralized_baseline}
\end{figure}
