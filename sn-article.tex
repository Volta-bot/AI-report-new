...
The advent of Deep Learning has revolutionized traffic prediction. Recurrent Neural Networks (RNNs), and specifically Long Short-Term Memory (LSTM) networks \cite{hochreiter1997lstm} and Gated Recurrent Units (GRUs), have become the standard for modeling temporal dependencies in time-series data. These models can effectively learn long-term patterns and handle non-linear relationships. More recently, Graph Neural Networks (GNNs) have been introduced to explicitly model the spatial topology of road networks. Approaches like Graph Convolutional Networks (GCNs) combined with temporal modules (T-GCN, ST-GCN) have achieved state-of-the-art performance by simultaneously capturing spatial correlations between road segments and temporal trends \cite{li2018dcrnn}. However, these centralized deep learning models require aggregating massive amounts of data at a central server, raising significant privacy and bandwidth concerns \cite{liu2020fedgru}.
...
\subsection{Overview}
This chapter details the methodology for the Edge-based Traffic Prediction system via Federated Learning (FL). The system is specifically designed to address the critical challenges of privacy, bandwidth, and latency in Intelligent Transportation Systems (ITS) \cite{tariq2024trustworthy,bharati2022federated}. By dividing different traffic regions into independent and isolated clients, the architecture ensures that raw traffic data never leaves the local environment, thereby guaranteeing robust data privacy.

To overcome the challenges of insufficient or poor-quality data at individual nodes \cite{qin2025fedcontrastive}, the system leverages FL to collaboratively train a global model. Furthermore, it addresses the issue of model parameter bloat by utilizing a lightweight architecture to reduce communication burdens, while incorporating mechanisms to handle data imbalance among clients \cite{Zhao2018FederatedNonIID}. This ensures that clients with large amounts of data do not disproportionately overshadow the individuality of other regions.
...
The \textbf{PeMS-BAY} dataset \cite{li2018dcrnn} contains traffic data from 325 sensors in the Bay Area, collected over six months (January--May 2017) at 5-minute intervals. Missing values are handled using linear interpolation \cite{Li2018MissingValueImputation}, and the data is normalized using Z-score normalization to improve model performance.
...
\end{figure}
The training loss decreases steadily across 160 rounds for 10 different edge clients, indicating successful model convergence \cite{mcmahan2017fedavg}. While different clients show varying convergence patterns due to data heterogeneity \cite{Zhao2018FederatedNonIID}, all remain stable without divergence, demonstrating effective federated learning.
...
\end{figure}
The validation MAE decreases over training rounds, showing continuous improvement of the model. Different clients achieve different levels of accuracy depending on traffic complexity, with some performing better in predictable environments and others showing higher error in more complex regions \cite{zhang2024personalized,dai2024road}. Differences in MAE across all clients demonstrate that federated learning preserves regional specialization while enabling global knowledge sharing.
...
The core of the empirical study focuses on validating the Federated Learning (FL) framework using the Multi-Horizon LSTM model \cite{hochreiter1997lstm} across two distinct urban environments: PeMS-BAY (California highway) and METR-LA (Los Angeles city streets) \cite{li2018dcrnn}. The results are categorized into convergence stability, temporal forecasting accuracy, and a comparative trade-off analysis.

\subsubsection{Convergence Analysis}

The efficiency of the Federated Averaging (FedAvg) algorithm \cite{mcmahan2017fedavg} was monitored over 50 communication rounds to ensure that distributed training can match the stability of centralized methods.
...
\textbf{Local Feature Integration:} From rounds 11 to 30, the learning rate stabilized as the model began to reconcile non-IID (non-independent and identically distributed) data \cite{Zhao2018FederatedNonIID} from different edge clients.
...
\textbf{Short-term (15 min):} Both datasets showed exceptional accuracy. PeMS-BAY maintained a MAPE of 3.88\%, demonstrating that highway speeds are highly predictable in the immediate future \cite{dai2025short}.
...
\textbf{Reliability:} Despite the increased difficulty of long-term forecasting, all MAPE values remained significantly below the 12\% threshold. This confirms that the Multi-Horizon LSTM architecture \cite{hochreiter1997lstm} successfully captures long-range temporal dependencies without suffering from vanishing gradients, a common issue in standard RNNs.

\textbf{Dataset Comparative Analysis:} PeMS-BAY (freeway) consistently outperformed METR-LA (urban) by approximately 35--40\% in MAPE \cite{li2018dcrnn}. This disparity is attributed to the inherent volatility of stop-and-go urban traffic versus the more stable flow of highway networks.
...
\textbf{Collaborative Boost via Knowledge Transfer:} Clients in irregular zones derived significant benefits from the federated framework \cite{liu2020fedgru}. By effectively borrowing structural traffic logic from stable downtown nodes, the Port Area client stabilized its MAPE at 8.91\%, whereas a local-only training approach would likely have resulted in errors exceeding 12\%.

\textbf{Scalability and Fairness:} The results confirm that even in inherently difficult regions, clients can achieve reasonable accuracy through federated knowledge access without sacrificing data sovereignty \cite{tariq2024trustworthy}.
...
\textbf{Accuracy Preservation:} The FL model successfully retained approximately 91.5\%--92\% of the accuracy achieved by the centralized counterpart \cite{liu2020fedgru,feng2024federated}. The minor absolute increase in MAPE (approximately 0.6\%--0.92\%) is categorized as a negligible privacy tax --- a small cost for the substantial security gains provided.

\textbf{Root Causes of the Performance Gap:}
\begin{itemize}
    \item \textit{Communication Constraints:} Unlike centralized training, which allows for continuous gradient updates with full precision, FedAvg \cite{mcmahan2017fedavg} relies on discretized aggregation at fixed communication rounds.
    \item \textit{Data Non-IID Effects:} Statistical heterogeneity across edge nodes \cite{Zhao2018FederatedNonIID} (e.g., complex traffic patterns in port areas) creates a residual disadvantage that global averaging cannot fully eliminate.
\end{itemize}

\textbf{Systemic Efficiency and Regulatory Compliance:}
\begin{itemize}
    \item \textit{Data Privacy:} Raw sensor data never leaves the origin point, ensuring that granular mobility patterns remain local. This privacy-by-design architecture inherently satisfies strict global regulations such as GDPR and CCPA \cite{tariq2024trustworthy}.
    \item \textit{Bandwidth Optimization:} The framework achieved a transformative 262$\times$ reduction in communication overhead. Instead of streaming massive high-frequency raw datasets, the system only transmits compressed model parameters (weights) every round \cite{mcmahan2017fedavg}.
    \item \textit{Edge Readiness and Latency:} With an optimized model size of only 20 KB, the architecture is fully compatible with resource-constrained edge devices \cite{dai2025short,bharati2022federated}. This enables real-time local inference with latencies below 100 ms, facilitating immediate proactive traffic management that centralized systems (burdened by round-trip query times) cannot match.
\end{itemize}
...
Beyond quantitative evaluation, it is important to analyze the practical strengths, operational limitations, and future scalability of the proposed framework.

\subsection{Strengths of the Proposed Framework}

The integration of Federated Learning (FL) with a lightweight Multi-Horizon LSTM architecture successfully balances predictive accuracy with operational efficiency. A primary advantage is robust privacy preservation; by keeping raw traffic data strictly localized via the FedAvg algorithm, the system minimizes attack surfaces and ensures strict regulatory compliance (e.g., GDPR). Additionally, exchanging only minimal model updates rather than streaming raw, high-frequency sensor data drastically reduces network bandwidth requirements. 

Furthermore, automated topological placement using Louvain clustering ensures that edge nodes capture locally relevant features and naturally partition the road network into structurally coherent traffic communities. This localized processing facilitates rapid, real-time inference on resource-constrained devices while maintaining predictive performance close to centralized baselines.

\subsection{Limitations and Challenges}

Despite its efficacy, the methodology presents several inherent limitations. A notable drawback is the performance degradation and slower convergence associated with extreme non-IID (Independent and Identically Distributed) data distributions across different edge clients. For instance, while highway environments exhibit relatively consistent predictions, complex urban networks show significantly higher variance. Clients located in irregular zones, such as port areas with heavy-vehicle movements, experience higher error rates due to highly volatile local traffic dynamics.

Moreover, the reliance on the standard FedAvg algorithm means that discretized aggregation at fixed communication rounds creates a slight performance gap compared to the continuous gradient updates of centralized training. Finally, the continuous transmission of model weights in every training round still introduces communication overhead that may burden IoT devices operating under unstable network conditions.

\subsection{Solutions and Improvements}

To overcome these challenges and further refine the framework, several algorithmic and architectural enhancements can be implemented. The statistical heterogeneity caused by non-IID data can be mitigated by replacing the standard FedAvg approach with advanced optimization algorithms such as FedProx or SCAFFOLD. These algorithms are specifically designed to penalize client drift and improve convergence stability across varying data distributions.

Furthermore, while the current Multi-Horizon LSTM effectively captures temporal dependencies with minimal computational cost, the model's accuracy in complex urban environments could be enhanced by investigating Spatio-Temporal Graph Neural Networks (ST-GNNs). Implementing ST-GNNs would allow the framework to model spatial correlations between interconnected road segments more explicitly, yielding more resilient and context-aware predictions in highly irregular traffic sectors.

\subsection{Future Research Directions}

Looking forward, this privacy-preserving edge architecture holds substantial potential for the scalable deployment of real-world Intelligent Transportation Systems (ITS). A highly promising research direction involves fusing multimodal data streams into the local training process. By incorporating auxiliary data such as roadside camera feeds, event schedules, and real-time weather conditions, the system's robustness and contextual awareness could be significantly improved.

Ultimately, this framework offers a secure and scalable blueprint that guarantees data sovereignty for municipalities while enabling accurate and proactive congestion management without exposing sensitive mobility patterns. Consequently, the proposed methodology establishes a strong foundational architecture for the next generation of smart city infrastructure.

\section{Conclusion}

This research presented a comprehensive, privacy-preserving Edge-based Traffic Prediction framework utilizing Federated Learning. By deploying a specialized, lightweight Multi-Horizon LSTM architecture at the network edge, the system successfully forecasted multi-horizon traffic conditions while maintaining low computational overhead. Furthermore, automated topological placement using Louvain clustering naturally addressed spatial disparities across the road network. Crucially, through the Federated Averaging algorithm, edge devices collaboratively trained the global model by exchanging only model parameters, ensuring that sensitive raw mobility data remained strictly localized.

Experimental evaluations on real-world benchmark datasets demonstrated the framework’s high efficacy. The proposed federated model achieved highly competitive predictive performance that closely approached ideal centralized baselines, while simultaneously delivering substantial improvements in communication efficiency and bandwidth optimization.

Although the system proves highly viable, certain operational limitations remain, particularly regarding convergence speeds when processing extreme data heterogeneity across diverse urban zones. To address these challenges, future research will focus on integrating advanced optimization algorithms to better handle statistical imbalances, alongside investigating Spatio-Temporal Graph modeling approaches. Ultimately, by establishing a secure and scalable architecture that guarantees data sovereignty, this methodology provides a foundational blueprint for proactive congestion management in next-generation Intelligent Transportation Systems.
\backmatter
